...
class LSTMRedup(object):
    ...
    def __init__(self, args, is_training, is_testing=False):
        ...
        self._cues = tf.placeholder(tf.int32, shape=[batch_size, num_steps])
        ...
        with tf.device(tf_device):
            ...
            with tf.variable_scope("rnnlm"):
                ...
                # character embedding
                cue_embedding = tf.get_variable("cue_embedding", [2, 200])
                with tf.device("/cpu:0"):
                    cues = tf.nn.embedding_lookup(cue_embedding, self._cues)

                if is_training and args.keep_prob < 1:
                    cues = tf.nn.dropout(cues, args.keep_prob)

                # split input into a list
                inputs = tf.split(1, num_steps, c2w_outputs)
                cue_vectors = tf.split(1, num_steps, cues)
                self.emb = c2w_outputs

                # concatenate input_vectors
                assert len(inputs) == len(cue_vectors)
                print(len(inputs))
                print(len(cue_vectors))
                final_inputs = []
                for i in range(len(inputs)):
                    # print(inputs[i].get_shape())
                    # print(cue_vectors[i].get_shape())
                    # final_input = tf.concat(2, [inputs[i], cue_vectors[i]])
                    final_input = inputs[i] + cue_vectors[i]
                    # print(final_input.get_shape())
                    final_inputs.append(final_input)

                if word_dim == rnn_size:
                    lm_inputs = [tf.squeeze(input_, [1]) for input_ in final_inputs]
                    self.emb = c2w_outputs
                else:
